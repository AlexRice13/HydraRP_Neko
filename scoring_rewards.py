...
def execute_code_safely(code: str, timeout: int = 30) -> dict:
    ...
    try:
        ...
        return {"status": "timeout", "output": f"超时 ({timeout}s)", "return_code": -1}
    except Exception as e:
        return {"status": "error", "output": f"系统异常: {str(e)}", "return_code": -1}
    finally:
...
def reward_code_interpreter(completion: str, prompt: str = "") -> float:
    ...
    judge_system_prompt = CODE_JUDGE_PRMPT
    judge_input = f"问题: {prompt}\n模型原始输出：{completion}代码: {code_content}\n运行输出: {exec_result['output']}"

    try:
        judge_res = call_judge_llm(judge_system_prompt, judge_input, "reward_code_interpreter")
...
def reward_image_gen(completion: str, prompt: str = "") -> float:
    target_self_re = re.compile(r"(自拍|自画像|你自己|selfie|portrait|picture of you|photo of you|你|玲奈|asahi|lina|me\b)", re.IGNORECASE)
    trait_species = re.compile(r"(cat\s*girl|neko|feline|ear|tail|猫娘|猫耳|兽耳|尾巴|猫咪)", re.IGNORECASE)
    ...
    trait_ear_color = re.compile(r"(orange|ginger|tangerine|yellow|橘|橙|黄)", re.IGNORECASE)
    trait_body = re.compile(r"(petite|small|short|155|girl|young|lady|teen|少女|萝莉|女孩|娇小|个子)", re.IGNORECASE)
    style_keywords = re.compile(r"(anime|manga|comic|illustration|realistic|photo|quality|light|detail|art|style|动漫|二次元|插画|绘画|写实|摄影|画质|光影|风格|作画)", re.IGNORECASE)
    ...
    token_proxy_len = len(prompt_text) if any("\u4e00" <= c <= "\u9fff" for c in prompt_text) else len(prompt_text.split())
    ...
    if is_self_request:
        ...
        if trait_eyes.search(prompt_text):
            traits_detected += 1
        if trait_hair.search(prompt_text) and re.search(r"(gold|blonde|yellow|金|黄)", prompt_text, re.IGNORECASE):
            traits_detected += 1
...
def reward_multi_turn_chat(completion: str, prompt: str = "") -> float:
    ...
    try:
        judge_input = f"[Dialogue History (ChatML)]:\n{prompt}\n\n[Current Response Analysis Target]:\n{completion}"
        judge_res = call_judge_llm(
            system_prompt=MULTI_TURN_JUDGE_PRMPT,
...
def calculate_length_penalty(completion: str, task_type: str = "") -> float:
    ...
    min_len, opt_min_len, opt_max_len, max_len = length_configs.get(task_type, length_configs["default"])
...
def calculate_thinking_format_penalty(completion: str, task_type: str = "") -> float:
    ...
    min_len, opt_min_len, opt_max_len, max_len = think_length_configs.get(task_type, think_length_configs["default"])
